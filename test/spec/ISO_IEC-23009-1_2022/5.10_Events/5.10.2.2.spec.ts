...
const {MPD, Period, AdaptationSet, EventStream, Event, InbandEventStream} = DASH;
...
describe('ISO_IEC-23009-1_2022/5.10.2.2', () => {
  ...
  test('InbandEventStream', () => {
    // The InbandEventStream type is defined based on the EventStream semantics, but the following restrictions apply:
    // * The @timescale attribute may be absent.
    // * The @presentationTimeOffset attribute shall be absent.
    // * The InbandEventStream element shall not contain any MPD Events, i.e. the Event element shall not be included in any InbandEventStream element.
    bothPass(`
      <?xml version="1.0" encoding="UTF-8"?>
      <MPD profiles="urn:mpeg:dash:profile:isoff-on-demand:2011" minBufferTime="PT2S">
        <Period duration="PT30S">
            ...
            new AdaptationSet({
              ...
              children: [
                new InbandEventStream({
                  schemeIdUri: 'urn:mpeg:dash:event:2012',
                  value: '1',
                  presentationTimeOffset: 270_000,
                }),
              ],
            }),
          ],
